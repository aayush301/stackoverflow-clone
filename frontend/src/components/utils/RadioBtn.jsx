    ...
    <>
      <label htmlFor={id} className='inline-flex items-center cursor-pointer group'>
        <span className='relative w-9 h-9 p-2 rounded-full transition group-hover:bg-gray-100 dark:group-hover:bg-gray-900'>
          <input type="checkbox" id={id} name={name} value={value} className='absolute peer w-full h-full top-0 left-0 opacity-0 cursor-pointer' onChange={handleRadioBtnClick} />
          <span className='w-5 h-5 p-[3px] z-10 absolute rounded-full border-2 border-gray-600' focusable="false" aria-hidden="true">
            {isChecked ? (
              <span className='w-2.5 h-2.5 z-20 absolute rounded-full bg-teal-600' focusable="false" aria-hidden="true"></span>
            ) : (
              <span className='w-2.5 h-2.5 z-20 absolute rounded-full bg-transparent' focusable="false" aria-hidden="true"></span>
            )}
          </span>
